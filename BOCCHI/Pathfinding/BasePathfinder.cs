...
public abstract class BasePathfinder(float returnCost = 300f, float teleportCost = 50f) : IPathfinder
{
    ...
    protected (float Cost, List<PathfinderStep> Steps) GetBestSteps(uint fromId, uint toId)
    {
        ...
        // fromId comes from the caller's node list (CarrotData.Data / TreasureData.Levels, both
        // hardcoded in C#), while these dictionaries come from the checked-in
        // Data/<zone>/precomputed_*_hunt_data.json, regenerated by hand from the debug panels.
        // The two key sets match today (verified: 25/25 carrots, 68/68 treasures), but nothing
        // enforces that - adding a node without regenerating the JSON would silently desync
        // them. NodeToNodeDistances above is already read through TryGetValue for the very same
        // key, so guard this one the same way and fall through to the return-to-camp branch
        // below rather than throwing out of the pathfinding chain.
        if (data.NodeToAethernetDistances.TryGetValue(fromId, out var aethernetList) && aethernetList.Count > 0)
        {
            var fromShard = aethernetList.OrderBy(x => x.Distance).FirstOrDefault();
